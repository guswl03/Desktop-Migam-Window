...
최종 갱신: 2026-08-30
현재 단계: 사진 배달·창 오르기 설정 v0.2.0 병합·배포 완료
전체 상태: PR #40과 main CI, Windows x64 NSIS 빌드, v0.2.0 GitHub Release 게시 완료; 실제 앱 회귀 점검은 후속 유지
...
| 2. 설정 | 진행 중 | 2026-08-23 |  | Codex | 기본 520×780 세로형 창과 좁은 화면 1열·제목 비겹침 자동 검증 완료, UI 재실행 복원 수동 확인 필요 |
...
| A. 사진 배달 연출 | 완료 | 2026-08-23 | 2026-08-30 | Codex | 일반 최대 430×350px·희귀 사진 85%, 배달 6초로 조정; 사진·저전력·희귀 사진 테스트 버튼 제거 |
| B. 창 등반·상단 보행 | 진행 중 | 2026-08-24 |  | Codex | 설정 ON/OFF와 창 위 8~15초 후 가까운 가장자리 자동 하강 구현; Windows 수동 게이트 대기 |
| 11. 최종 검증 | 완료 | 2026-08-30 | 2026-08-30 | Codex | 프런트 66개·Rust 51개·typecheck·build·fmt·Clippy·NSIS·main CI 통과, v0.2.0 게시 |

상태 값: `대기`, `진행 중`, `차단`, `완료`, `제외`

## 현재 작업

- 작업: v0.2.0 병합·GitHub Release 게시와 배포 기록
- 시작 시각: 2026-08-30
- 종료 시각: 2026-08-30
- 결과: PR #40 병합, main CI 통과, v0.2.0 태그·정식 릴리스와 Windows x64 설치 파일 게시
- 후속: 실제 Windows에서 설정 재실행 복원과 여러 창에서 8~15초 자동 하강 체감 회귀 점검

## 오늘 완료

- [x] 사진 배달 최대 크기를 520×420px에서 430×350px로 줄이고 최소 크기도 260×208px로 조정
- [x] 희귀 사진만 일반 표시 크기의 85%로 축소하고 일반 사진 크기 유지
- [x] 사진 배달 이동 시간을 8초에서 6초로 단축
- [x] 우클릭 사진·저전력 테스트와 설정 희귀 사진 이스터에그 버튼을 모든 빌드에서 제거
- [x] 개발·배포용 Vite 번들에서 세 테스트 문구가 0건임을 확인
- [x] 창 오르기 설정을 기본 ON으로 추가하고 저장·실시간 반영·OFF 충돌 차단 연결
- [x] 창 위 착지 후 8~15초 뒤 가까운 가장자리로 걸어가 기존 낙하 물리로 자동 복귀
- [x] 자동 하강 중 다른 창을 만나도 재등반하지 않도록 하강 목표 우선 처리
- [x] 동작하지 않던 펫 크기 입력과 도움말을 설정 화면에서 제거하고 내부 스키마 값은 호환성을 위해 보존
- [x] 설정창 기본 크기를 520×780으로 변경하고 좁은 화면의 제목·빌드 정보 겹침을 제거
- [x] 프런트 66개·Rust 51개 테스트, TypeScript, Vite build, rustfmt, Clippy 통과
- [x] 앱·패키지·README 링크를 v0.2.0으로 동기화하고 Windows x64 NSIS 설치 파일 생성
- [x] PR #40과 main CI 통과 후 v0.2.0 정식 릴리스 및 설치 파일 게시
...
- [x] 20~40분 무작위 자동 배달 구현, 후속 배포 정리에서 우클릭 테스트 버튼 제거
...
| 2026-08-30 | 희귀 사진 크기·배달 속도 조정 | 통과 | 일반 사진 크기는 유지하고 희귀 사진만 85%로 축소, 이동 시간을 6초로 단축; 프런트 13파일 66개·typecheck·production build 통과 |
| 2026-08-30 | v0.2.0 최종 릴리스 | 통과 | PR #40·main CI, 프런트 66개·Rust 51개·typecheck·production/NSIS release build·fmt·Clippy 통과; Windows x64 설치 파일과 SHA-256 게시 |
