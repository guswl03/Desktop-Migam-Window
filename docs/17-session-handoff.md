...
- 설정의 펫 영역에 `창 위로 올라가기`를 추가했다. 기존 사용자 동작 보존을 위해 기본값과 구 설정 마이그레이션 값은 ON이며, OFF 저장 시 새 창 충돌은 등반을 시작하지 않는다. 실행 중 저장되는 `settings://saved` 이벤트도 펫 런타임에 즉시 반영한다.
- 창 위에 착지하면 8~15초 체류 시간을 한 번 예약한다. 시간이 지나면 idle/walking 상태와 무관하게 가까운 가장자리로 걸어가며, 가장자리에서는 걷던 방향의 약한 수평 속도를 유지한 채 기존 중력·낙하·착지 상태로 복귀한다. 내려가기로 결정한 뒤에는 다른 창에 재등반하지 않는다.
- `visualScalePercent`는 저장·검증만 되고 실제 펫 창·스프라이트에서 사용되지 않았다. 혼동을 주는 `펫 크기 (%)` 입력과 도움말은 제거했으며, 기존 설정 파일 호환성을 위해 내부 스키마 필드와 저장된 값은 보존한다.
...
- 창 위 착지 후 8~15초가 지나면 현재 위치에서 가까운 가장자리로 걸어간 뒤 기존 낙하 물리로 바닥에 복귀한다.
...
- 평상시 20~40분마다 사진 4장 중 하나를 무작위 배달한다. Focus뿐 아니라 짧은 휴식·긴 휴식·일시정지 중에도 시작하지 않으며 긴급 중지 시 즉시 숨긴다.
- 펫 우클릭 WinDbg 메뉴의 수동 사진 배달 테스트 버튼은 배포 정리 단계에서 제거했다.
- Desktop Goose처럼 힘들어 보이도록 6초 동안 전진·조금 밀림·버팀을 반복한다. 당기는 4자세를 900ms로 재생하고 몸 기울기와 사진 흔들림을 함께 사용한다.
- 일반 사진은 원본 비율을 유지하며 약 260~430px 범위로 표시하고 `real-heogeodeongseu` 희귀 사진만 계산된 크기의 85%로 줄인다. 외곽 제목 표시줄·테두리·여백과 보이는 `X`가 없으며, 사진 우측 상단 46×46px의 투명 영역을 누르면 닫힌다.
...
- 설정의 펫 영역에 `자동 사진 배달 사용`을 추가했다. 기본값은 ON이며 OFF일 때 20~40분 예약 배달을 막는다. 수동 사진 배달 테스트 버튼은 제거했다.
...
- 저전력 이벤트 전용 자산은 `images/characters/gamjabot/extra/frames/low-battery-carry-sheet-v1.png`이며 3×2 투명 시트다. 우클릭 수동 테스트 버튼은 배포 정리 단계에서 제거했다.
...
- `visualScalePercent`는 기존 설정 파일 호환성을 위해 스키마에만 유지하며 설정 화면에서는 노출하지 않는다.
